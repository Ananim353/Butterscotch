...
//
// Overridable per platform: a particle costs ~48 bytes where GMLReal is float, so the desktop default
// would reserve 393 KB per system. The PSP build cuts this down in CMakeLists; DELTARUNE's heaviest
// system peaks at 89 live particles, so even a tenth of the default is generous.
